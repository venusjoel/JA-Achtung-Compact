...
module psram_controller #(
    ...
    parameter integer BURST_BYTES   = 4,
...
// Display reads use 4-byte bursts. Game reads/writes use one byte, because one
// framebuffer byte contains eight one-bit pixels.
localparam [3:0] READ_NIBBLES_LAST = 4'd7;  // 4-byte read = 8 nibbles
localparam [3:0] BYTE_NIBBLES_LAST = 4'd1;  // 1-byte transfer = 2 nibbles
...
function [3:0] addr_nibble;
    input [16:0] addr_byte;
    input [3:0] remaining;
    begin
        case (remaining)
            4'd5: addr_nibble = {3'b000, addr_byte[16]};
            4'd4: addr_nibble = addr_byte[15:12];
            4'd3: addr_nibble = addr_byte[11:8];
            4'd2: addr_nibble = addr_byte[7:4];
            default: addr_nibble = addr_byte[3:0];
        endcase
    end
endfunction
...
reg [2:0]  gap_cnt;
reg [16:0] addr_byte_lat;
reg [3:0]  wr_lo_nibble;
    ...
    if (!rst_n) begin
            ...
            if (sclk_f) begin
                if (nibble_cnt == 4'd0) begin
                    if (is_read) begin
                        ...
                        state       <= S_DATA_WR;
                        started     <= 1'b1;
                        nibble_cnt  <= BYTE_NIBBLES_LAST;
                        wr_lo_nibble <= i_wdata[3:0];
                        sio_out_reg <= i_wdata[7:4];
                    end
                end else begin
                    sio_out_reg <= addr_nibble(addr_byte_lat, nibble_cnt);
                    nibble_cnt  <= nibble_cnt - 4'd1;
                end
            end
        end
        ...
        S_WAIT: begin
            ...
            if (sclk_f) begin
                if (nibble_cnt == 4'd0) begin
                    state          <= S_DATA_RD;
                    nibble_cnt     <= byte_access ? BYTE_NIBBLES_LAST : READ_NIBBLES_LAST;
        ...
        S_DATA_RD: begin
            ...
            if (sclk_r) begin
                if (nibble_cnt[0]) begin
                    ...
                    // Full display bursts shift so byte 0 ends in o_rdata[7:0].
                    if (byte_access)
                        o_rdata <= {{(DATA_WIDTH-8){1'b0}}, rx_hi_nibble, sio_in};
                    else
