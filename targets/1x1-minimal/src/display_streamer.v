...
module display_streamer #(
    ...
    input  wire        disp_ack,
    output wire        pixel_occupied
);

    // Framebuffer is packed as 1 bit per 4 horizontal pixels. With the fixed
    // 4-byte burst, each fetch covers 32 cells or 128 visible pixels.
    ...
    wire burst_last_pixel = (FRAME_W == 64) ? (&pixel_idx[5:0]) : (&pixel_idx);
    wire [4:0] cell_bit_idx = pixel_idx[6:2];
    assign pixel_occupied =
        (active_video && display_shift_valid) ?
        display_shift[cell_bit_idx] : 1'b0;
    ...
    wire current_burst_last_pixel =
        ...
        burst_last_pixel;
    wire [11:0] next_fetch_burst_after_done = next_fetch_burst + 1'b1;
    wire can_request_burst =
        !disp_req && (!display_shift_valid || (!vblank && !next_burst_valid));
    assign disp_addr = {3'b000, next_fetch_burst, 2'b00};
